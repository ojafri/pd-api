...
import geocode, * as adapters from 'geocodr'
import {sleep, getArg, getRequiredArg, getJsonArg} from 'helpr'
...
let geocoder = adapters[getArg('geocoder', {dflt: 'geocodio'})]
// eslint-disable-next-line camelcase
const geoParams = getJsonArg('geoParams', {dflt: {api_key: 'bfc5c9e6dc557e7ecb72757ce9b07596c757ff7'}})
...
dbg('geocoder.url=%o', geocoder.url)
...
export default (async function () {
  try {
    ...
    dbg(
      ...
      inputName, outputName, batchSize, query, sleepMillis, dryRun
    )

    dbg('address fields: line-1=%o, city=%o, state=%o, zip=%o', line1Field, cityField, stateField, zipField)

    const addresses = await input.aggregate(
      [
        ...steps,
        ...(limit ? [{$limit: limit}] : [])
      ],
...
async function geocodeAddresses({addresses, getAddress, output}) {
  ...
  while (begin < addresses.length) {
    ...
    if (batchSize > 1) {
      ...
      coordinates = await geocode(batch, geocoder)
    } else {
      coordinates = [await geocode(getAddress(addresses[begin]), geocoder)]
    }
