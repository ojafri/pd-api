...
import {evalInContext, setState, asTemplate} from 'test-helpr'
import {setArgDefault} from 'helpr'
...
export default function (context) {
  return function () {
    ...
    this.When(/^we run the '([^']+)' ingester with environment:$/, async (ingesterString, envString) => {
      const ingester = evalInContext({js: asTemplate(ingesterString), context})
