...
import {pretty, getArg, getJsonArg, deepClean, stringify} from 'helpr'
...
export default function (
  ...
  const dbg = debug(`app:${path.basename(process.argv[1], '.js')}`)
  ...
  return async function () {
    ...
    const cursorTimeoutMs = getArg('cursorTimeoutMs', {dflt: _.get(config, 'mongo.cursorTimeoutMs', 300000)})
    ...
    try {
      ...
      outputIndices && await createIndices({indices: outputIndices, db, collectionName: _outputName})
      ...
      const cursor = await input.aggregate(
        _steps,
        {allowDiskUse: true},
        {cursor: {batchSize}}
      ...
      if (postProcessor) {
        ...
        do {
          ...
          if (record) {
            ...
            try {
              const result = await postProcessor({output, record, source, date})
              ...
              dbg('caught error=%o, continuing...', err)
            ...
            if (timer.count() % thresh === 0) {
              ...
              dbg('heap-used-mb=%o', process.memoryUsage().heapUsed / 1e6)
