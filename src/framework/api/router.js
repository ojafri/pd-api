...
import {isZip5, parseBoolean} from 'helpr'
import getData from '../data/data'
import {getName, isIdField} from '../data/helper'
import constants from '../data/constants'
import xformQuery from '../data/xform-query'
import {getZipCoordinates} from './helper'
import {configuredProvider as geoProvider} from './geo-provider'
import {dbgreq} from './express-helper'
import {operatorMatcher} from './mongo-xform-query'

export default function (opts) {
  const dbg = debug(`app:api:get-router(${getName(opts)})`)
  ...
  async function _index({req, res, next, query}) {
    try {
      ...
      const promises =
        parseBoolean(query.includeCount) ?
...
export async function getQuery({query, opts = {}}) {
  return xformQuery(
    query,
    {
      ...
      xforms: {
        ...opts.xforms,
        nearAddress: async ({result, key, value}) => {
