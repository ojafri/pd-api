...
import {getDb, parseParam, findOne, toDotNotation} from 'mongo-helpr'
import {pretty, getType, getWithTypes, stringify} from 'helpr'
...
export default function (opts) {
  ...
  function getCreate({collectionName, isValid, createHook, idHook}) {
    ...
    return async function ({data = {}, context = {}}) {
      ...
      const collection = db.collection(collectionName)
      const _data = await runHook({hook: opts.dataHook, flowKey: 'data', data, db, context, mode, opts})
      if (idHook) {
        _data[constants.ID_FIELD] = await idHook({data: _data, db, opts, context})
      }
      // dbg('create: validated data=%o', stringify(_data))
      const result = createHook ? await createHook({data: _data, db, opts, context}) : await collection.insertOne(_data)
  ...
  function getUpdate({collectionName, isValid, idHook, updateHook, isUpsert, postUpdateHook}) {
    ...
    return async function ({id, data = {}, context = {}}) {
      dbg('update: id=%o, type=%o, data=%o, context=%o, is-upsert=%o', id, getType(id), data, context, isUpsert)
      assert(data, 'data required')
      const db = await getDb()
      isValid && assert(await isValid({...opts, db, data, context, mode}))
      const collection = db.collection(collectionName)
      ...
      result && postUpdateHook && await postUpdateHook({result, id: actualId, filter, data, context, opts, db, update: $set})
      ...
      return _.reduce(
        ...
        (result, value, key) => {
          if (!['offset', 'limit', 'sort', 'nearLat', 'nearLon', 'nearMiles'].includes(key)) {
            if (!key.startsWith('$')) { // don't process mongo directives
  ...
  async function getSteps({opts, query, context, sort}) {
    const {distanceField = 'distance', stepsHook, steps = [], postStepsHook, postSteps = [], docField: prefix} = opts
    ...
    const _steps = stepsHook ? await stepsHook({steps, query, context, queryPostProcessor, sort, prefix}) : steps
    result.push(..._steps)
    ...
    const _postSteps = postStepsHook ? await postStepsHook({postSteps, query, context, queryPostProcessor, sort, prefix}) : postSteps
