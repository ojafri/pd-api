...
import {getNextSequence, assertNone, getDb, sanitizeKeys} from 'mongo-helpr'
import {join, isLike, SEPARATOR, deepClean} from 'helpr'
import constants from './constants'
import xformQuery from './xform-query'

const dbg = debug('app:shared:framework-helper')
...
export function registerEvent({mode, opts}) {
  ...
  if (hook) {
    return function (opts) {
      ...
      return Promise.all(
        asFunctionArray(hook).map(elt => elt({...opts, mode}))
      )
    }
...
export function matcherStepsHook(matchers) {
  return async function ({steps, query, context, queryPostProcessor}) {
    ...
    //
    const _query = _.transform(
      await queryPostProcessor({query, context}),
      (result, val, key) => {
        if (!key.startsWith('$')) {
          result[key] = val
        }
...
export function getSyntheticResult({result, data, path}) {
  ...
  if (result) {
    ...
    return {
      matchedCount: result.lastErrorObject.n,
      upsertedCount: result.lastErrorObject.upserted ? 1 : 0,
      modifiedCount: !value || isLike({actual: value, expected: data}) ? 0 : 1,
      result: {ok: result.ok, n: result.lastErrorObject.n},
      id: result.lastErrorObject.upserted || _.get(value, '_id'),
      original: result.value
    }
  }
}

export async function captureDataChange({target, source, user, date, data, update, mode = constants.MODES.update}) {
  // dbg('capture-data-change: args=%j', arguments[0])
  const db = await getDb()
  const result = await db.collection(`${target}History`).insertOne(
    deepClean(
      {
        source,
        user,
