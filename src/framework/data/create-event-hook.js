...
const dbg = debug('app:shared:create-event-hook')
...
export default function ({target, path, fields, filterHook}) {
  ...
  const defaultFilterIdPath = join([...stripPlaceholders(path).split('.').slice(0, -1), constants.ID_FIELD])
  ...
  return async function ({opts, id, data, db, context}) {
    dbg(
      ...
      target, getName(opts), id, stringify(data), stringify(context)
    )
