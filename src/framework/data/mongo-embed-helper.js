...
import {findOne} from 'mongo-helpr'
import {stringify, getType, getWithTypes, findDeepIndices, join} from 'helpr'
import {isIdField, isCreate, isUpsert, isUpdate, isDelete, isUpdateOrUpsert, getSyntheticResult} from './helper'
import constants from './constants'

const dbg = debug('app:shared:mongo-embed-helper')

const {MODES} = constants

export default function ({contextPath, isAssociative}) {
  assert(_.isArray(contextPath) && !_.isEmpty(contextPath), 'contextPath is a required non-empty array')
  if (isAssociative) {
    assert(
      contextPath.length === 2,
      'exactly two contextPath entries required when isAssociative'
    )
  }
  ...
  async function upsert({id, data, db, opts, context, mode}) {
    dbg(
      ...
      id, getType(id), data, opts, context, mode
    )
    ...
    const pushPath = _.transform(
      ...
      (result, elt) => {
        ...
        if (Array.isArray(array)) {
          ...
          if (elt.idPath) {
            dbg('array=%j, id-path=%o, id-field=%o, id-target=%o', array, elt.idPath, idField, idTarget)
