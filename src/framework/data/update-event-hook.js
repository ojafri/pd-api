...
import {stringify, join, findDeepIndices} from 'helpr'
import {toDotNotation} from 'mongo-helpr'
import debug from 'debug'
import {getName, getIdPath, stripPlaceholders} from './helper'

const dbg = debug('app:data:update-event-hook')
...
export default function ({target, path, fields}) {
  ...
  return async function ({id, data, db, mode, opts}) {
    dbg('entity=%o, target=%o, id=%o, data=%o, mode=%o', getName(opts), target, id, stringify(data), mode)
    ...
    if (!_.isEmpty(dottedPicks)) {
      ...
      const result = await db.collection(target).updateMany(
        filter,
        {$set}
      )
      // dbg('result=%o', result)
...
function updateDeepNestedEventHook({target, path, fields}) {
  ...
  return async function ({opts, id, data, db, context, mode}) {
    dbg(
      ...
      getName(opts), target, id, stringify(data), context, mode
    )
      ...
      if (record) {
        ...
        if (array) {
          ...
          if (indices) {
            ...
            const result = await db.collection(target).updateOne(
              {_id: record._id},
              {$set}
            )
            assert(result.modifiedCount > 0, `modifications expected for record with id=${id}`)
          }
        }
      }
