...
test('createHook', async t => {
  ...
  let result = await createHook(
    {data: {_id: 'n1', foo: 'bar'}, db, opts, context: {clientId: 'c1'}}
  )
...
test('updateHook', async t => {
  ...
  let result = await updateHook(
    {id: networkId, data: {foo: 'baz'}, db, opts, context: {clientId: 'c1', networkId}}
  )
...
test('deleteHook', async t => {
  ...
  result = await db.collection(collectionName).find({_id}).toArray()
  dbg('result=%s', stringify(result))
