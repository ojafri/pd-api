...
import {initDb, initFixture} from 'mongo-test-helpr'
import {getDb, findOne} from 'mongo-helpr'
import getData from '../../../../src/framework/data/data'
import getEmbedHooks from '../../../../src/framework/data/mongo-embed-helper'
import {mongoIdHook} from '../../../../src/framework/data/helper'
...
test('update: embedded', async t => {
  ...
  await initFixture({db, collectionName, docs: [{_id: id, nesters: [{_id: embeddedId, foo: 'bar'}]}]})
...
test('upsert: embedded: create', async t => {
  ...
  await initFixture({db, collectionName, docs: [{_id: id}]})
  let result = await embeddedData.upsert({id: embeddedId, data: {foo: 'bar'}, context: {'test._id': id}})
  t.truthy(result.result.ok)
  t.truthy(eventHookFlags.create)
  result = await findOne({collectionName})
  t.truthy(result)
...
test('upsert: embedded: update', async t => {
  ...
  const embeddedId = 'embedded-id-1'
  await initFixture({db, collectionName, docs: [{_id: id, nesters: [{_id: embeddedId, foo: 'bar'}]}]})
  let result = await embeddedData.upsert({id: embeddedId, data: {foo: 'baz'}, context: {'test._id': id}})
  t.truthy(result.result.ok)
  t.truthy(eventHookFlags.update)
  result = await findOne({collectionName})
  t.truthy(result)
